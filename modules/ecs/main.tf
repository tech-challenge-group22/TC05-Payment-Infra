...
resource "aws_alb" "alb_payment" {
  name            = "${var.prefix}-alb-payment"
...
resource "aws_alb_listener" "payment" {
  load_balancer_arn = "${aws_alb.alb_payment.arn}"
  port              = "3000"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.prefix}_payment_web_cpu_utilization_high"
